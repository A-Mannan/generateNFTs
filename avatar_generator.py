...
class AvatarGenerator:
    ...
    def save_image(self, image: Image.Image, index: int = 0):
        image_index = str(index).zfill(4)
        image_file_name = f"avatar_{image_index}.png"
        image_file_path = os.path.join(self.output_path, image_file_name)
        image.save(image_file_path)

    def generate_avatar(self, numOfAvatars: int):
        for i in range(numOfAvatars):
